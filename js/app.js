...
function selectPartner(num) {
  ...
  var badge = document.getElementById('activePartner');
  badge.textContent = 'P' + num;
  var ps = document.getElementById('partnerSelect');
  ...
  setTimeout(function() {
    ...
    setTimeout(function() { document.getElementById('appShell').style.opacity = '1'; }, 50);
    initApp();
  }, 400);
}
...
async function renderTimeline() {
  ...
  feed.innerHTML = items.map(function(item) {
    var photoHtml = '';
    if (item.photo) {
      photoHtml = '<img src="' + escapeHtml(item.photo) + '" alt="" />';
    }
    var audioHtml = '';
    if (item.audio) {
